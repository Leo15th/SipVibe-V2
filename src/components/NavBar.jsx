...
export default function NavBar({links, shopName, logInClick, registerClick, user, onLogout, cart}){
    ...
    return(
        <nav className={`fixed left-0 right-0 top-0 z-40 flex justify-between items-center backdrop-blur-sm ${scrolled ? "border-b bg-black/70" : "bg-black"} border-b-white/30 px-5 lg:px-10 py-4 h-[72px]`}>
            {user?
            (   
                <>
                    { isCart ?
                        (
                            <>
                                <div className="text-white">
                                    <Button btnName="Back" onClickListener={()=>navigate("/menu")}/>
                                </div>
                            </>
                        ):(
                        <>
                            ...
                            <div className="flex items-center gap-4">
                                <div className="relative">
                                    <i onClick={()=>navigate("/cart")} className="fa-solid fa-cart-shopping text-white text-2xl hover:cursor-pointer"></i>
                                    {cart && cart.length>0 &&(
                ...
                <>
                    <h3 className="text-2xl md:text-3xl lg:text-3xl font-extrabold underline text-white hover:gold-text-shadow-md cursor-pointer font-mono hover:scale-105"><RouterLink to="/">{shopName}</RouterLink></h3>
                    {
                        isHome? (
                            <>
